...
function getData() {
    table = $("#myTable").DataTable({
        ...
        columns: [
            ...
            {
                render: function (data, type, row, meta) {
                    return `<a href="/upload/file_sk_pangkat/${row.file_sk}">Lihat File</a>`
                }
            },
            {
                render: function (data, type, row, meta) {
...
$('#modal').on('show.bs.modal', function (event) {
    ...
    if (recipient) {
        ...
        modal.find('#golongan').val(cokData[0].golongan);
...
form.onsubmit = (e) => {
    ...
    axios({
        method: 'post',
        url: formData.get('id') == '' ? '/store-kepangkatan' : '/update-kepangkatan',
        data: formData,
    })
        .then(function (res) {
            //handle success         
