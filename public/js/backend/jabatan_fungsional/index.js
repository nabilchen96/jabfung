...
function getData() {
    table = $("#myTable").DataTable({
        ...
        columns: [
            ...
            { data: "nomor_sk" },
            { data: "terhitung_mulai_tanggal" },
            { data: "status_pegawai" },
            {
                render: function (data, type, row, meta) {
                    return `<a href="/upload/file_sk/${row.file_sk}">Lihat File</a>`
                }
            },
            {
                render: function (data, type, row, meta) {
...
$('#modal').on('show.bs.modal', function (event) {
    ...
    if (recipient) {
        ...
        modal.find('#jabatan_fungsional').val(cokData[0].jabatan_fungsional);
        modal.find('#nomor_sk').val(cokData[0].nomor_sk);
        modal.find('#terhitung_mulai_tanggal').val(cokData[0].terhitung_mulai_tanggal);
...
form.onsubmit = (e) => {
    ...
    axios({
        method: 'post',
        url: formData.get('id') == '' ? '/store-jabatan-fungsional' : '/update-jabatan-fungsional',
        data: formData,
    })
        .then(function (res) {
            //handle success         
...
hapusData = (id) => {
    Swal.fire({
        ...
        if (result.value) {
            axios.post('/delete-jabatan-fungsional', {
                id
            })
                .then((response) => {
